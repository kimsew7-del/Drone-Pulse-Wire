...
def fetch_europepmc_source(source: dict[str, Any], sources: list[dict[str, Any]]) -> list[NewsItem]:
    params = {
        "query": (source.get("query") or '"drone" OR "autonomous flight" OR "artificial intelligence"') + " sort_date:y",
...
def collect_from_sources(
    ...
    for source in sources:
        ...
        try:
            ...
            logger.warning("소스 %s 네트워크 오류: %s", source.get("name"), exc)
            record_source_stat(source_stats, source, 0, "network_error")
            continue
        except Exception as exc:
            logger.warning("소스 %s 수집 오류: %s", source.get("name"), exc, exc_info=True)
