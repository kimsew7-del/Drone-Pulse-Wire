...
def build_corpus_vocabulary(
    ...
    if window_hours is not None:
        ...
        filtered = []
        for item in items:
            ts = parse_iso_datetime(item.timestamp)
            if ts and ts >= cutoff:
...
def assign_auto_categories(
    ...
    for cat in categories:
        ...
        for item in items:
            if item.id in article_ids and label not in item.auto_categories:
    ...
    for item in sorted_news:
        ...
        if matched_idx is None and item.canonical_key:
            ...
            if matched_idx is None and len(tokens) >= 3:
                ...
                for tok in tokens:
                    if tok in token_to_groups:
                        candidate_idxs |= token_to_groups[tok]
                for gidx in candidate_idxs:
                    if is_duplicate(item, groups[gidx][0]):
                        matched_idx = gidx
                        break

        if matched_idx is not None:
            groups[matched_idx].append(item)
        else:
            gidx = len(groups)
            groups.append([item])
            url_to_group[item.url] = gidx
            if item.canonical_key:
                key_to_group[item.canonical_key] = gidx
                for tok in item.canonical_key.split():
                    if tok not in token_to_groups:
                        token_to_groups[tok] = set()
                    token_to_groups[tok].add(gidx)
