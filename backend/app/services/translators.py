...
def translate_with_ollama(text: str, mode: str = "summary", model_override: str | None = None) -> str | None:
    ...
    try:
        ...
        logger.debug("Ollama 번역 실패: %s", text[:50], exc_info=True)
        return None
...
def translate_with_papago(text: str) -> str | None:
    ...
    try:
        ...
        logger.debug("Papago 번역 실패: %s", text[:50], exc_info=True)
        return None
...
def translate_with_libretranslate(text: str) -> str | None:
    ...
    try:
        ...
        logger.debug("LibreTranslate 번역 실패: %s", text[:50], exc_info=True)
        return None
...
def translate_to_korean_gtx(text: str) -> str:
    ...
    try:
        ...
        logger.debug("GTX 한국어 번역 실패: %s", text[:50], exc_info=True)
    return text
